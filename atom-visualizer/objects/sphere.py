...
class Sphere(Drawable):
    def __init__(
        self, shader_type="phong", color=None, radius=1.0, nsegments=32, name=None
    ):
        # fmt: off
        """
        ...
        color = color if color else Color.GRAY
        ...
        self.colors = np.tile([color], int(self.vertices.size)).astype(np.float32)
